...
hace red real: el destino staging (`/rest/v1/aportes`,
`/rest/v1/source_watermarks` y `/rest/v1/sources`) se intercepta con
...
- Construir el payload del aporte usando los contratos de
  ...
  `dedup_hash`, `dedup_version`, `block_keys`, `content_hash`, `source_id`
...
- `external_id` es determinista (fingerprint v1 para Event/AcopioCenter,
  `deterministic_id` para Person). PostgREST hace upsert por
  `(source_id, external_id)` via
  `/rest/v1/aportes?on_conflict=source_id,external_id` y
  `Prefer: resolution=merge-duplicates,return=minimal`, así que re-correr la
  misma fuente no duplica (idempotencia).
- Resolver `source_slug` (`source.id` del YAML) a UUID real en `public.sources`
  antes de construir payload. Si no se puede resolver en modo activo, el
  exporter no envía el registro.
- Enviar en lotes (batch) de hasta 100 registros por request. Cada batch
  exitoso (200/201) cuenta como enviado; un batch fallido se registra como
  error y bloquea el avance del watermark. Como se usa `return=minimal`,
  PostgREST no reporta duplicados por fila y `staging_duplicates` queda en 0
  por diseño.
- Avanzar el watermark de la fuente (`POST /rest/v1/source_watermarks?on_conflict=source_slug` con
  `Prefer: resolution=merge-duplicates`, body
  `{"source_slug": "...", "watermark_at": "<ISO>"}`)
...
El SQL operativo del PR crea el rol dedicado, lo otorga a `authenticator`,
agrega el índice único `(source_id, external_id)` y define policies RLS para
leer `sources`, escribir `aportes` y leer/escribir `source_watermarks`.
...
En cambio, 401/403 en `get_watermark` aborta la fuente para evitar un backfill
completo por un problema de auth/RLS.

Modo dry-run silencioso: si faltan `SUPABASE_URL`,
`SUPABASE_PUBLISHABLE_KEY` y `SUPABASE_INGEST_JWT`, el exporter queda
deshabilitado, no abre cliente HTTP (cero red), loguea a INFO lo que enviaría y
termina con `staging_sent=0` sin error. Si la configuración está parcial,
loguea ERROR y también entra en dry-run para evitar envíos con credenciales
incompletas.
...
`aportes` de Supabase vía PostgREST (`POST /rest/v1/aportes?on_conflict=source_id,external_id`,
ver "Capa 4 — Staging exporter"). Vercel queda fuera del ingest. Cada aporte
es un objeto JSON en snake_case con `external_id` determinista para
idempotencia.
...
```json
{
  ...
  "source_id": "uuid-v4-de-public-sources",
...
1. UTF-8, JSON válido.
2. Batch upsert via PostgREST con `apikey` + `Authorization: Bearer`.
3. `external_id` determinista y unique operativo `(source_id, external_id)`.
...
10. Trazabilidad hacia fuente (`source_id`, resuelto desde `sources.slug`).
